import React, { View, Text, StyleSheet, TouchableOpacity, ScrollView, Alert, TextInput, Image, Button } from "react-native"
...
const CreateHomeWork = () => {
  ...
  const createHomeWork = async () => {
    if (!selectOption.class) {
      ...
      return;
    } else if (!selectOption.section) {
      Alert.alert("Info", "Please select section");
      return;
    } else if (!selectOption.subject) {
      Alert.alert("Info", "Please select subject");
      return;
    }
    // else if (selectFileOpt.data?.name === "Youtube Url" && urlList.length == 0) {
    ...
    try {
      ...
      const res = await Services.formMethod(apiRoot.createHomework, formData);
      if (res.status === "success") {
      ...
      console.log('Error in createHomeWork:', err);
      Alert.alert("Info", "Something went wrong while uploading homework.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
                      ...
                      <View>
                        {showYoutubeUrlView &&
                          <View style={{ flexDirection: 'row', borderWidth: 1, borderRadius: 4, borderColor: userData.data.colors.hoverTheme, backgroundColor: SWATheam.SwaWhite, marginVertical: 5, padding: 4 }}>
                            ...
                            <TouchableOpacity style={{ paddingVertical: 5, display: inputVal ? 'flex' : 'none' }} onPress={() => getUrls()}>
                              <Ionicons name={"send"} size={25} color={userData.data.colors.mainTheme} />
                            </TouchableOpacity>
                          </View>
                        }
      ...
      {selectFileOpt.status &&
        <View style={styles.selectFieldPopUp}>
          ...
          <View style={{ backgroundColor: '#fff', padding: 8, maxHeight: 300, borderTopRightRadius: 6, borderTopLeftRadius: 6 }}>
