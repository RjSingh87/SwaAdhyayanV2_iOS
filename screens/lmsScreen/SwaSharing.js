...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, ScrollView, Image, Button, PermissionsAndroid, StatusBar, Platform, Alert } from "react-native"
import DatePicker from 'react-native-date-picker'
import DocumentPicker from 'react-native-document-picker';
// import { pick, pickSingle } from '@react-native-documents/picker';
...
const SwaSharing = ({ navigation }) => {
  ...
  const [shareGroup, setShareGroup] = useState({ list: null, type: null, status: false })
  ...
  const [selectGroup, setSelectGroup] = useState({ data: null, radioID: null })
  ...
  function selectGroupOpt(item, navigation) {

    console.log(item, 'check selected Item')
  ...
  function removeMemberFrmShareGroup(userRefID, name = "") {
    ...
    Services.post(apiRoot.removeMemberFromSharingGroupApp, payload)
      .then((res) => {
        if (res.status == "success") {
          Alert.alert("Alert", name + ' ' + res.message)
  ...
  function classDataValidate(type) {
    if (type == 'class') {
      ...
      if (selectClassText == 'Select Class') {
        ...
        Alert.alert("Info!", 'Please Select Class')
      } else {
        SetShowClassPopUp(true)
        getSectionList()
      }
  ...
  function sharePost() {
    ...
    console.log('jfjfjfformData', formData)

    Services.formMethod(apiRoot.sharePostApp, formData)
      .then((res) => {
        console.log(formData, "FValue.")

        console.log(JSON.stringify(res), '-------- check response----------')
  ...
  const copyToPublic = async (filePath) => {
    const ext = filePath.split('.').pop();
    const publicPath = `/storage/emulated/0/Download/shared_${Date.now()}.${ext}`;

    const exists = await RNBlobUtil.fs.exists(publicPath);
    if (exists) {
      await RNBlobUtil.fs.unlink(publicPath);
    }

    await RNBlobUtil.fs.cp(filePath, publicPath);
    return publicPath;
  };

  const downloadFile = async (docPath, type) => {
    setLoading(true);
    const { fs } = RNBlobUtil;
    const date = new Date();
    ...
    const localPath = `${fileDir}/sharing_${date.getTime()}${fileExtension}`;

    // Detect MIME type

    let mimeType = '';
    if (fileExtension === '.pdf') mimeType = 'application/pdf';
    else if (['.jpg', '.jpeg', '.png', '.gif'].includes(fileExtension)) mimeType = 'image/*';
    else if (['.mp4', '.ogg'].includes(fileExtension)) mimeType = 'video/*';
    else if (fileExtension === '.mp3') mimeType = 'audio/*';
    else mimeType = '*/*';
    ...
    try {
      ...
      if (Platform.OS === 'android' && Platform.Version >= 30) {
        copyToPublic(filePath)
        try {
          const publicPath = await copyToPublic(filePath);
          await Share.open({
            url: 'file://' + publicPath,
            type: publicPath.endsWith('.pdf') ? 'application/pdf' : 'image/*',
          });
        } catch (err) {
        ...
        try {
          ...
          console.log('⚠️ File open error:', err);
          Alert.alert('Error', 'File downloaded but cannot be opened.');
        }
      }
    ...
    Services.post(apiRoot.getAppSharingGroup, payload)
      .then((res) => {
        if (res.status == "success") {
          const data = res.data
          setShareGroup((prev) => {
            return { ...prev, list: data, type: 'group', status: true }
          })
        } else {
          setShareGroup((prev) => {
            return { ...prev, status: false }
    ...
    <SafeAreaProvider style={{ paddingTop: insets.top, marginBottom: insets.bottom, backgroundColor: userData.data.colors.mainTheme }}>
      {loading ?
        ...
        <View style={{ flex: 1, backgroundColor: userData.data.colors.liteTheme }}>
          ...
          <ScrollView style={{ flex: 1, padding: 10 }}>
            {userData.data.userTypeID != 5 && userData.data.userTypeID != 6 &&
              <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginBottom: 10 }}>
                <TouchableOpacity style={{ backgroundColor: '#198754', paddingHorizontal: 10, paddingVertical: 10, borderRadius: 50, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }} onPress={() => { getList(), setShowCreateGroup(true), setSelectOptText('Select Group To Share'), setGroupFileImg('Select Image'), setSelectGroup({ data: null, radioID: null }), setCreateGroupImg('') }}>
            ...
            {showPostList && userData.data.userTypeID != 5 && userData.data.userTypeID != 6 ?
              <View>
                <SelectionBox getListItem={getList} selectedField={selectGroup?.data?.groupName} type="group" placeholder="Select Group" />
                ...
                <View style={{ flexDirection: 'row', marginVertical: 15 }}>
                  ...
                  <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                    <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center' }} onPress={() => setSelectDocBtn(true)}>
                      <Feather name={"file-text"} size={25} color={selectDocBtn ? SWATheam.SwaBlack : '#0d6efd'} />
                ...
                {selectImgBtn &&
                  <View style={{ marginBottom: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row', borderWidth: 1, borderColor: SWATheam.SwaGray, borderRadius: 4, backgroundColor: SWATheam.SwaWhite }}>
                      <View style={{ flex: 1, padding: 8, paddingLeft: 10 }}>
                        <Text style={{ color: SWATheam.SwaBlack }}>{postImgName}</Text>
                      </View>
                      <TouchableOpacity style={{ padding: 6, backgroundColor: SWATheam.SwaGray, borderTopRightRadius: 4, borderBottomRightRadius: 4 }} onPress={() => getBrowseImg('image')}>
                ...
                {selectVideoBtn &&
                  <View style={{ marginBottom: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row', borderWidth: 1, borderColor: SWATheam.SwaGray, borderRadius: 4, backgroundColor: SWATheam.SwaWhite }}>
                      ...
                      <TouchableOpacity style={{ padding: 8, backgroundColor: SWATheam.SwaGray, borderTopRightRadius: 4, borderBottomRightRadius: 4 }} onPress={() => getBrowseImg('video')}>
                        <Text style={{ color: SWATheam.SwaWhite }}>Browse</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                ...
                {selectAudioBtn &&
                  <View style={{ marginBottom: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row', borderWidth: 1, borderColor: SWATheam.SwaGray, borderRadius: 4, backgroundColor: SWATheam.SwaWhite }}>
                      ...
                      <TouchableOpacity style={{ padding: 8, backgroundColor: SWATheam.SwaGray, borderTopRightRadius: 4, borderBottomRightRadius: 4 }} onPress={() => getBrowseImg('audio')}>
                        <Text style={{ color: SWATheam.SwaWhite }}>Browse</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                ...
                {selectDocBtn &&
                  <View style={{ marginBottom: 10 }}>
                    ...
                    <View style={{ flexDirection: 'row', borderWidth: 1, borderColor: SWATheam.SwaGray, borderRadius: 4, backgroundColor: SWATheam.SwaWhite }}>
                      ...
                      <TouchableOpacity style={{ padding: 8, backgroundColor: SWATheam.SwaGray, borderTopRightRadius: 4, borderBottomRightRadius: 4 }} onPress={() => getBrowseImg('docx')}>
                        <Text style={{ color: SWATheam.SwaWhite }}>Browse</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                            ...
                            return (
                              <View style={{ borderWidth: 0.5, borderRadius: 7, borderColor: SWATheam.SwaGray, marginBottom: 5, paddingHorizontal: 5, backgroundColor: SWATheam.SwaWhite }} key={index}>
...

                                {
                                  item.videoFileNPath ?
                                    <View style={{ flexDirection: 'row', borderWidth: 0.5, borderColor: SWATheam.SwaGray, borderRadius: 5, marginVertical: 5, marginHorizontal: 10 }}>
                                      <TouchableOpacity style={{ flex: 1, justifyContent: 'center', alignItems: 'center', padding: 5 }} onPress={() => viewDoc(postList.domainName + item.videoFileNPath, 'video',)}>
                                        <Video
          ...
          {shareGroup.status &&
            <BottomDrawerList listItem={shareGroup} closeModule={closeModal} getSelectedItem={selectGroupOpt} selectedField={selectGroup} />
          }
            ...
            <View style={{ position: 'absolute', top: 0, left: 0, backgroundColor: SWATheam.SwaWhite, flex: 1, width: '100%', height: '100%' }}>
              ...
              <View style={{ paddingHorizontal: 10, marginTop: 20, flex: 1, paddingBottom: 40 }}>
                <View style={{ borderBottomWidth: 1, borderColor: greyClr, paddingBottom: 10 }}>
                  ...
                  <View style={{ flexDirection: 'row', borderWidth: 1, borderColor: SWATheam.SwaGray, borderRadius: 4 }}>
                    <View style={{ flex: 1, padding: 8, paddingLeft: 10 }}>
                      <Text style={{ color: SWATheam.SwaBlack }}>{groupFileImg}</Text>
                    </View>
                    <TouchableOpacity style={{ padding: 8, backgroundColor: userData.data.colors.mainTheme, borderTopRightRadius: 4, borderBottomRightRadius: 4 }} onPress={() => getBrowseImg('image')}>
                      <Text style={{ color: SWATheam.SwaWhite }}>Browse</Text>
                    </TouchableOpacity>
                  </View>
          ...
          {editGroupData &&
            <View style={{ position: 'absolute', top: 0, left: 0, backgroundColor: SWATheam.SwaWhite, flex: 1, width: '100%', height: '100%' }}>
              ...
              <View style={{ position: 'absolute', bottom: 0, width: '100%', justifyContent: 'center', alignItems: 'center', backgroundColor: SWATheam.SwaWhite, borderTopWidth: 0.5, paddingVertical: 5 }}>
                <TouchableOpacity style={{ backgroundColor: userData.data.colors.mainTheme, paddingHorizontal: 10, paddingVertical: 5, borderRadius: 5 }} onPress={() => { setEditGroupData(false), getList() }}>
                  <Text style={{ color: SWATheam.SwaWhite, fontWeight: '500' }}>Close</Text>
                </TouchableOpacity>
              </View>
            </View>
